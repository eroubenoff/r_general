...
# start: start year
# end: end year
# interval: age intervals 


Gompertz <- function(df, start, end, interval) {
...
for(i in age) {
  temp <- life_table %>% subset(Age==i) %>% select(varname)
  print(temp[,1])
...
spain_gompertz <- Gompertz(df = spain , start = 1830, end = 1910, interval = 10)
test1 <-spain_gompertz %>% ggplot() + geom_text(mapping = aes(x=log.alpha,y=beta, label=years), color="red") 
